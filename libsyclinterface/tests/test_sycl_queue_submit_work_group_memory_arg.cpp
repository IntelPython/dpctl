//===-- test_sycl_queue_submit_work_group_memory_arg - Test work group mem ===//
//
...
///
/// \file
/// This file contains tests for kernel submit using the work_group_memory
/// SYCL extension.
//===----------------------------------------------------------------------===//
...
{
    if (!DPCTLWorkGroupMemory_Available()) {
        GTEST_SKIP()
            << "Skipping work-group-memory test since the compiler does "
    ...
    auto wgm = DPCTLWorkGroupMemory_Create(wgm_sz);
    ASSERT_TRUE(wgm != nullptr);
    auto *wgm_raw = unwrap<RawWorkGroupMemory>(wgm);
    ASSERT_TRUE(wgm_raw != nullptr);
    ASSERT_TRUE(wgm_raw->nbytes == wgm_sz);
    void *args_1d[NARGS] = {unwrap<void>(a), wgm};
    ...
    void *args_2d[NARGS] = {unwrap<void>(a), wgm};
    ...
    void *args_3d[NARGS] = {unwrap<void>(a), wgm};
