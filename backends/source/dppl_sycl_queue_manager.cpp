...
public:
  ...
  static QVec *init_active_queues() {
    ...
    try {
      ...
      } else if (BE == backend::level_zero && DevTy == info::device_type::gpu) {
  ...
  static QVec &get_opencl_cpu_queues() {
    static QVec *queues = init_queues(backend::opencl, info::device_type::cpu);
    return *queues;
  }

  static QVec &get_opencl_gpu_queues() {
    static QVec *queues = init_queues(backend::opencl, info::device_type::gpu);
    return *queues;
  }
  ...
  static __dppl_give DPPLSyclQueueRef getQueue(DPPLSyclBackendType BETy,
  ...
  static __dppl_give DPPLSyclQueueRef setAsDefaultQueue(
      DPPLSyclBackendType BETy, DPPLSyclDeviceType DeviceTy, size_t DNum);

  static __dppl_give DPPLSyclQueueRef pushSyclQueue(DPPLSyclBackendType BETy,
                                                    DPPLSyclDeviceType DeviceTy,
                                                    size_t DNum);
 ...
 * be used for that purpose.
 */
__dppl_give DPPLSyclQueueRef QMgrHelper::getQueue(DPPLSyclBackendType BETy,
                                                  DPPLSyclDeviceType DeviceTy,
                                                  size_t DNum) {
  queue *QRef = nullptr;
  ...
  case DPPLSyclBackendType::DPPL_OPENCL | DPPLSyclDeviceType::DPPL_CPU: {
    ...
    if (DNum >= cpuQs.size()) {
      // \todo handle error
      std::cerr << "OpenCL CPU device " << DNum << " not found on system.\n";
      return nullptr;
    }
  ...
  case DPPLSyclBackendType::DPPL_OPENCL | DPPLSyclDeviceType::DPPL_GPU: {
    ...
    if (DNum >= gpuQs.size()) {
      // \todo handle error
      std::cerr << "OpenCL GPU device " << DNum << " not found on system.\n";
      return nullptr;
    }
  ...
  case DPPLSyclBackendType::DPPL_LEVEL_ZERO | DPPLSyclDeviceType::DPPL_GPU: {
    ...
    if (DNum >= l0GpuQs.size()) {
      // \todo handle error
      std::cerr << "Level-0 GPU device " << DNum << " not found on system.\n";
      return nullptr;
    }
 ...
 */
__dppl_give DPPLSyclQueueRef QMgrHelper::setAsDefaultQueue(
    DPPLSyclBackendType BETy, DPPLSyclDeviceType DeviceTy, size_t DNum) {
  queue *QRef = nullptr;
 ...
 * purpose.
 */
__dppl_give DPPLSyclQueueRef QMgrHelper::pushSyclQueue(
    DPPLSyclBackendType BETy, DPPLSyclDeviceType DeviceTy, size_t DNum) {
  queue *QRef = nullptr;
  auto &activeQ = get_active_queues();
  if (activeQ.empty()) {
