...
struct TestDPPLSyclProgramInterface : public ::testing::Test {
  ...
  TestDPPLSyclProgramInterface() {
    ...
    PRef = DPPLProgram_CreateFromOCLSource(CtxRef, CLProgramStr, CompileOpts);

    // Create a program from a SPIR-V file
    std::ifstream file{"./multi_kernel.spv", std::ios::binary | std::ios::ate};
    auto fileSize = std::filesystem::file_size("./multi_kernel.spv");
    file.seekg(0, std::ios::beg);
    std::vector<char> buffer(fileSize);
    file.read(buffer.data(), fileSize);
    PRef2 = DPPLProgram_CreateFromOCLSpirv(CtxRef, buffer.data(), fileSize);
  }
