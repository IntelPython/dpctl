...
extern void cpp_blackscholes(DPPLSyclQueueRef q_ptr, size_t n_opts, T* params, T* callput) {
	...
	if (params_type != sycl::usm::alloc::shared) {
	    throw std::runtime_error("Input option_params to cpp_blackscholes is not a USM-shared pointer.");
	}
    }
    {
	sycl::usm::alloc callput_type = sycl::get_pointer_type(callput, ctx);
	if (callput_type != sycl::usm::alloc::shared) {
	    throw std::runtime_error("Input callput to cpp_blackscholes is not a USM-shared pointer.");
	}
    }
	...
	[&](sycl::handler &cgh){
		...
		[=](sycl::id<1> idx) {
		    ...
		    data_t mr = -opt_rate, sig_sig_two = two * opt_volatility * opt_volatility;
...
void cpp_populate_params(DPPLSyclQueueRef q_ptr, size_t n_opts, T* params, T pl, T ph, T sl, T sh, T tl, T th, T rl, T rh, T vl, T vh, int seed) {
	...
	[&](sycl::handler &cgh) {
		...
		[=](sycl::item<1> idx) {
		    ...
		    oneapi::mkl::rng::device::philox4x32x10<n_params_next_pow2> engine(seed, j);
		    oneapi::mkl::rng::device::uniform<T> distr;

		    sycl::vec<T, n_params_next_pow2> res = oneapi::mkl::rng::device::generate(distr, engine);
