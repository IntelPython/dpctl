...
the set of :ref:`devices <basic_concepts_devices>` available on the system.
...
device, a :exc:`ValueError` is raised.
...
2. Out of that list the devices that support half-precision floating-point
   arithmetic are selected.
3. A "score" computed using the SYCL8 runtime's default device scoring logic that is
    stored in :attr:`dpctl.SyclDevice.default_selector_score` is used to select a single device.
