...
def _raise_driver_error(fname, errcode):
    e = DpplDriverError(
        "Could not find an OpenCL Driver. Ensure OpenCL \
                         driver is installed."
    )
    e.fname = fname
    e.code = errcode
...
def _raise_unsupported_kernel_arg_error(fname):
    e = UnsupportedTypeError(
        "Type needs to be DeviceArray or a supported " "ctypes type."
    )
    e.fname = fname
    raise e
...
class DeviceArray:
    ...
    def __init__(self, env_ptr, arr):
        ...
        retval = lib.create_dp_rw_mem_buffer(env_ptr, self._buffSize, self._buffObj)
        if retval == -1:
...
class Program:
    def __init__(self, device_env, spirv_module):
        ...
        retval = lib.create_dp_program_from_spirv(
            device_env.get_env_ptr(), spirv_module, len(spirv_module), self._prog_t_obj
        )
        if retval == -1:
            _logger.warning("OpenCL Error Code  : %s", retval)
            _raise_driver_error("create_dp_program_from_spirv", -1)

        retval = lib.build_dp_program(device_env.get_env_ptr(), self._prog_t_obj[0])
        if retval == -1:
            _logger.warning("OpenCL Error Code  : %s", retval)
    ...
    def __init__(self, arg, void_p_arg=False):
        ...
        if void_p_arg is True:
            ...
            retval = lib.create_dp_kernel_arg(
                self.ptr_to_arg_p, ffi.sizeof(self.ptr_to_arg_p), self.kernel_arg_t
            )
            if retval:
            ...
            if isinstance(arg, DeviceArray):
                ...
                if getattr(arg, "__module__", None) == "ctypes":
                    ...
                    retval = lib.create_dp_kernel_arg(
                        self.ptr_to_arg_p, ctypes.sizeof(arg), self.kernel_arg_t
                    )
                    if retval:
                        _raise_driver_error("create_dp_kernel_arg", -1)
                else:
...
class DeviceEnv:
    ...
    def copy_array_to_device(self, array):
        ...
        if isinstance(array, DeviceArray):
            ...
            isinstance(array, ndarray) or getattr(array, "__module__", None) == "ctypes"
        ):
    ...
    def create_device_array(self, array):
        ...
        if not (
            (
                isinstance(array, ndarray)
                or getattr(array, "__module__", None) == "ctypes"
            )
        ):
...
def enqueue_kernel(device_env, kernel, kernelargs, global_work_size, local_work_size):
    ...
    if local_work_size:
        l_work_size_array = ffi.new("size_t [" + str(len(local_work_size)) + "]")
    else:
    ...
    retval = lib.set_args_and_enqueue_dp_kernel(
        device_env.get_env_ptr(),
...
def igpu_context(*args, **kwds):
    ...
    lib.set_curr_env(runtime.get_runtime_ptr(), runtime.get_gpu_device().get_env_ptr())
...
def cpu_context(*args, **kwds):
    ...
    lib.set_curr_env(runtime.get_runtime_ptr(), runtime.get_cpu_device().get_env_ptr())
    device_env = runtime.get_current_device()
    yield device_env
