...
struct dpctl_capi
{
    ...
    dpctl_capi(dpctl_capi const &) = delete;
    void operator=(dpctl_capi const &) = delete;
    ~dpctl_capi(){};

    static auto get()
    {
        static dpctl_capi *api = new dpctl_capi();
        return api;
    }
...
private:
    ...
    friend std::unique_ptr<dpctl::detail::dpctl_capi>
    std::make_unique<dpctl::detail::dpctl_capi>();
}; // struct dpctl_capi
} // namespace detail
} // namespace dpctl
